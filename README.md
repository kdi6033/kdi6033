...
본 라인업은 다양한 **IoT 센서(온·습도, CO₂, 조도, 전압, 전류, 거리 등)**를 안정적으로 수집·처리하는 것을  
최우선으로 설계되었으며, Wi-Fi, Bluetooth, RS-485, MQTT 통신을 통해  
현장 기기 및 네트워크와 실시간으로 연결됩니다.

각 디바이스는 센서로부터 수집된 데이터를 **On-Device AI 기반으로 현장에서 즉시 분석·판단**하고,  
그 결과를 바탕으로 PLC 출력, 액추에이터, 외부 제어 시스템을  
**Edge 단에서 직접 제어**합니다.  
이를 통해 단순 모니터링을 넘어  
**입력(Sensor) → 판단(AI) → 출력(Control)** 이 하나의 연속된 제어 루프로 동작합니다.

이 구조는 산업용 IoT 센서, 액추에이터, 타사 PLC 및 설비를 유기적으로 연결하여  
클라우드 의존을 최소화하면서도 **빠르고 안정적인 Physical AI 기반 자동화 시스템**을 구현합니다.
